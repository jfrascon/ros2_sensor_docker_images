...
# This script is aimed to launch a Docker container that contains the ROS2 drivers for
# UMX IMU, Livox Gen2 LiDAR, and RoboSense LiDAR.
# If the mode is 'automatic', this script will start the container and automatically launch the drivers.
# If the mode is 'manual', this script will start the container, but you will need to connect to the container and run
# the script 'exec_manual_launch.sh', to launch the drivers.
...
# Example:
#  ./<this_script> multi_sensor:humble automatic --env ROS_LOCALHOST_ONLY=1
...
# Example:
#  ./<this_script> multi_sensor:jazzy manual --namespace myns --robot-name myrobot --ros-domain-id 20 \
#    --rmw-implementation rmw_fastrtps_cpp --env ROS_AUTOMATIC_DISCOVERY_RANGE=LOCALHOST \
#    --env ROS_STATIC_PEERS='192.168.0.1'

usage() {
    cat <<EOF
Usage:
  $(basename ${BASH_SOURCE[0]}) <img_id> <mode> [options]
...
Options:
  ...
  --env env_var             Additional environment variable in KEY=VALUE format
                            (e.g., --env ROS_LOCALHOST_ONLY=1).
                            Use it for variables not covered by explicit flags.
                            You can provide multiple --env options.
...
# --namespace VALUE --robot-name VALUE --ros-domain-id VALUE --rmw-implementation VALUE --env env_var_1 --env env_var_2 ... -- img_id mode
...
# Create a temporary Docker compose file to pass extra environment variables to all services.

# Example:
#   --env ROS_LOCALHOST_ONLY=1
#   --env ROS_AUTOMATIC_DISCOVERY_RANGE=LOCALHOST
# Resulting extra-env snippet (applied to each service):
#   environment:
...
# Build the list of YAML environment entries that will be injected into each service.
# Visual example (per service):
#   environment:
#     ROS_LOCALHOST_ONLY: '1'
env_lines=()

# Input: env_vars[] = raw KEY=VALUE strings from --env.
# Output: env_lines[] = validated and escaped KEY=VALUE entries.
...
# If there is at least one entry, generate a per-service extra-env file that repeats the same environment block
# for umx_imu_srvc, livox_gen2_lidar_srvc, and robosense_lidar_srvc, then include it via an extra -f file.
if [ "${#env_lines[@]}" -gt 0 ]; then
    env_vars_file="$(mktemp -p /tmp dc_extra_env_XXXXXX.yaml)"
    printf "services:\n" >"${env_vars_file}"

    for service in umx_imu_srvc livox_gen2_lidar_srvc robosense_lidar_srvc; do
        printf "  %s:\n    environment:\n" "${service}" >>"${env_vars_file}"
    ...
    # Include the generated extra-env file so Compose merges it into each service.
...
print_banner_text "=" "Launching multi-sensor drivers in a Docker container of the image '${IMG_ID}' in '${mode}' mode"
...
# Therefore, you can run inside the container rviz2 or rqt to visualize the sensor data.
xhost +local:
