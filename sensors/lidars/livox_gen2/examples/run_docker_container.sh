...
# This script launches a Docker container with the Livox Gen2 ROS2 driver using Docker compose files.
...
Options:
  ...
  --env KEY=VALUE           Environment variable for the container (repeatable)
                            ...
                            TOPIC_REMAPPINGS (default: empty)
...
if [ "${EXAMPLE}" == "1" ]; then
    USER_CONFIG_FILE_HOST="${SCRIPT_DIR}/example_1.front_livox_mid360.json"
    PARAMS_FILE_HOST="${SCRIPT_DIR}/example_1.front_livox_mid360.yaml"
else
    USER_CONFIG_FILE_HOST="${SCRIPT_DIR}/example_2.front_back_livox_mid360.json"
    PARAMS_FILE_HOST="${SCRIPT_DIR}/example_2.front_back_livox_mid360.yaml"
fi

[ ! -f "${PARAMS_FILE_HOST}" ] && {
    echo "Error: Parameter file '${PARAMS_FILE_HOST}' does not exist or is not a file" >&2
    exit 1
}
...
env_file="$(mktemp -p /tmp livox_gen2_env_XXXXXX.env)"
...
print_banner_text "=" "Launching Livox Gen2 in a Docker container of image '${IMG_ID}' (example ${EXAMPLE}) in '${mode}' mode"
