...
# This script launches the Livox Gen2 example container using Docker Compose.
...
# After parsing options, the remaining arguments are the positional ones.
IMG_ID="${1:-}"
mode="${2:-}"
...
# Determine the directory of the current script to reference the Docker Compose files.
SCRIPT_DIR="$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")" && pwd)"

# Create the array with the docker compose files to be used + the flag -f for each of them.
compose_files=(-f "${SCRIPT_DIR}/docker_compose_base.yaml" -f "${SCRIPT_DIR}/docker_compose_mode_${mode}.yaml")
...
if [ -n "${DISPLAY:-}" ]; then
    ...
    compose_files+=(-f "${SCRIPT_DIR}/docker_compose_gui.yaml")
    xhost +local:
fi

print_banner_text "=" "Launching Livox Gen2 in a Docker container of image '${IMG_ID}' in '${mode}' mode"

IMG_ID="${IMG_ID}" docker compose "${compose_files[@]}" up -d
