...
usage() {
    cat <<EOF
Usage:
  $(basename "${BASH_SOURCE[0]}") [--source-dir <dir>] [--remote-ref <ref>] [--clone-dir <dir>] [--l4t] [--help | -h]

Description:
  Clone librealsense2 and run the appropriate kernel patch script (Ubuntu LTS HWE or Jetson L4T).
...
Options:
  ...
  --l4t               Use the Jetson L4T patch script instead of the Ubuntu LTS HWE script.
...
SHORT_OPTS="h"
LONG_OPTS="help,source-dir:,remote-ref:,clone-dir:,l4t"
...
# Patch the kernel modules using the librealsense scripts directory.
if [ "${USE_L4T}" -eq 1 ]; then
    patch_script="${SOURCE_DIR}/scripts/patch-realsense-ubuntu-L4T.sh"
    log "Patching kernel modules for Jetson L4T"
else
    patch_script="${SOURCE_DIR}/scripts/patch-realsense-ubuntu-lts-hwe.sh"
