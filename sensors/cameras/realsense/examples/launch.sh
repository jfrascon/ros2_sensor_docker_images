...
# This script is aimed to launch an execution example of an Intel Realsense camera inside a docker container by
# executing a docker compose file.
...
# 2. Since the container where the realsense binary runs does not have GUI support you can't use 'rviz2' to visualize
#    the data. You can use another container that you may have at hand that has GUI support, so you can run 'rviz2'
#    there and visualize the data.
#    Remember to use the same DDS middleware and ROS_DOMAIN_ID in both containers so they can communicate.

# Check if docker compose service is running and healthy.
...
usage() {
    echo "Usage: ${script_name} <ros_distro>" >&2
    exit 1
}
...
if ! docker image inspect realsense:${ros_distro} 1>/dev/null 2>&1; then
    echo "Error: Docker image 'realsense:${ros_distro}' not found. Make sure to build it before running this script"
    exit 1
fi

print_banner_text "=" "Launching example ${example} with ROS distro ${ros_distro}"

compose_file="example_1.realsense_docker_compose_${ros_distro}.yaml"
docker compose -f "${compose_file}" up -d

# Check if the service is running and healthy.
check_compose_service realsense_srvc "${compose_file}"
