...
# This script launches a Docker container with the UMX ROS2 driver using Docker compose files.
...
UM_MODEL_DEF="7"
NODE_OPTIONS_DEF="name=umx,output=screen,emulate_tty=True,respawn=False,respawn_delay=0.0"
...
usage() {
    cat <<EOF2
Usage:
  $(basename "${BASH_SOURCE[0]}") <img_id> <mode> [--um-model 6|7] [--env KEY=VALUE]...
...
Options:
  --um-model VALUE          UM IMU model: 6 or 7 (default: ${UM_MODEL_DEF})
  --env KEY=VALUE           Environment variable for the container (repeatable)
                            ...
                            TOPIC_REMAPPINGS (default depends on UM_MODEL)
...
SHORT_OPTS="h"
LONG_OPTS="help,env:,um-model:,um_model:"
...
if [ "${UM_MODEL}" != "6" ] && [ "${UM_MODEL}" != "7" ]; then
    echo "Supported UM IMU models are 6 and 7 only" >&2
    usage
    exit 1
fi

# Check if the image '${IMG_ID}' exists locally.
...
# Assign default values to environment variables passed to the container, and override them with --env.
NAMESPACE="${NAMESPACE_DEF}"
ROBOT_NAME="${ROBOT_NAME_DEF}"
ROS_DOMAIN_ID="${ROS_DOMAIN_ID_DEF}"
TOPIC_REMAPPINGS_DEF="imu/data:=um${UM_MODEL}/data,imu/mag:=um${UM_MODEL}/mag,imu/rpy:=um${UM_MODEL}/rpy,imu/temperature:=um${UM_MODEL}/temperature,imu/reset:=um${UM_MODEL}/reset"
TOPIC_REMAPPINGS="${TOPIC_REMAPPINGS_DEF}"
NODE_OPTIONS="${NODE_OPTIONS_DEF}"
LOGGING_OPTIONS="${LOGGING_OPTIONS_DEF}"

if [ "${UM_MODEL}" == "6" ]; then
    PARAMS_FILE="${SCRIPT_DIR}/um6_params.yaml"
else
    PARAMS_FILE="${SCRIPT_DIR}/um7_params.yaml"
fi

[ ! -f "${PARAMS_FILE}" ] && {
    echo "Error: Parameter file '${PARAMS_FILE}' does not exist or is not a file" >&2
    exit 1
}
...
# Recognized keys are assigned to specific variables, while unrecognized ones are collected in 'extra_env_vars' to be
# passed as-is to the container.
for env_kv in "${env_vars[@]}"; do
    ...
    LOGGING_OPTIONS)
        ...
        ;;
    UM_MODEL)
        echo "Warning: ignoring --env UM_MODEL=... (model is taken from --um-model)" >&2
        ;;
    PARAMS_FILE)
        echo "Error: --env PARAMS_FILE is not allowed; it is managed from --um-model" >&2
        exit 1
        ;;
    IMG_ID)
...
env_file="$(mktemp -p /tmp umx_env_XXXXXX.env)"
...
print_banner_text "=" "Launching UMX in a Docker container of image '${IMG_ID}' (UM${UM_MODEL}) in '${mode}' mode"
...
# If you are in a Wayland session, XWayland should already be active.
# If it is not active in your Wayland session, you will need to enable it.
# Remember that '/tmp/.X11-unix:/tmp/.X11-unix:ro' must be mounted in docker_compose_base.yaml.
# Warning: 'xhost +local:' allows any local user to access the X server.
xhost +local:

IMG_ID="${IMG_ID}" PARAMS_FILE="${PARAMS_FILE}" ENV_FILE="${env_file}" docker compose "${compose_files[@]}" up -d
